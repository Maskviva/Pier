 ...
 *
 * 文件头的设计说明在 `plot_confine.h`。这里只补三条实现上的取舍。
 *
 ...
 * # 二、组根缓存整表清空，不做失效分析
 *
 * 合并表是**整表替换**推过来的（见 `setPlotMerges` 的说明），所以「哪些组根
 * 变了」这个问题没有便宜的答案：加一条合并边可能把两个几百块的组并成一个，
 * 组内每一块的根都变。整表清空是 O(1) 的，重建是懒的（下次查到才走图），
 * 而合并操作本身每服每天也就几十次。
 *
 * # 三、组遍历有上界，撞上界时**保守拒绝**
 *
 * 上界和模组侧一致（4096）。但两侧撞到上界时的行为**故意不同**：
 *
 *   * 模组侧那个组遍历是给标题去重用的，走到哪算哪，代价是超大组里可能多弹
 *     一次标题；
    ...
    namespace
    {
        ...
        )
        {
            ...
            catch (...)
            {
                ...
                // 不依赖这一步。所以这里**故意**不升级成 error：每 tick 一条
