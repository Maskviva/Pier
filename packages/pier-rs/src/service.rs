...
//! # 相对上一代的改动
//!
//! 上一代 `call()` 返回裸 `String`，于是每个调用点都要重复同一段样板。业务
//! 仓库里的真实写法：
//!
//! ```ignore
//! let Ok(body) = levilamina::service::call("plot:worlds", "{}") else { return 0 };
//! let Ok(v) = serde_json::from_str::<serde_json::Value>(&body) else { return 0 };
//! let Some(arr) = v.as_array() else { return 0 };
//! let ps = w.get("plotSize").and_then(Value::as_i64).unwrap_or(0);
//! //                                                 ^^^^^^^^^^^^
//! //  「没有这个键」和「它是 0」在这里变成了同一个答案
//! ```
//!
//! 这段样板在 rsw 里出现了十几次，每次都在同一个地方漏掉同一件事。所以这里
//! 提供 [`call_json`]：直接反序列化成你要的类型，解析失败是**一种明确的错误**，
//! 不是 `unwrap_or` 的兜底值。
//!
//! 另外把错误分了类（[`CallError`]）：「没有这个服务」和「服务说不行」是两回
//! 事 —— 前者多半是依赖没装或名字拼错，后者是业务拒绝。上一代把它们混在一
//! 个字符串里。
