...
//! 宿主现在会在解析不到时报错并列出相近 id（契约 §5.3），但那要等到运行时。
//! 常量能把它提前到编译期。
//!
//! # 两类 id
//!
//! * **注册表事件**（`ll::event::…`）—— LeviLamina 自己发的，Pier 按名字转发。
//!   写全名最稳；宿主也接受唯一后缀（`PlayerChatEvent`），但后缀在上游新增
//!   同名事件时会变歧义，所以这里一律给全名。
//! * **合成事件**（裸名字）—— Pier 用原生 detour 造的，LeviLamina 没有。
//!   它们补的正是保护类模组最需要、而 LL 覆盖不到的那些点。
//!
//! 每条都注明**能不能取消**：不可取消的事件上调 `Event::cancel()` 是无害的
//! no-op，但你会以为拦住了。这一栏值得看一眼。
