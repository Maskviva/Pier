...
//! # 这个模块想解决的具体问题
//!
//! 上一代只把原始能力递出来（订阅拿一个句柄、回调里给一段 SNBT），剩下的
//! 每个模组自己补。补出来的东西长这样 —— 这是业务仓库里真实存在的四个文件：
//!
//! ```text
//! rsw-perms-event/src/payload.rs    100 行   从 NbtValue 里取键
//! rsw-perms-event/src/hit.rs        217 行   把「玩家 + 坐标 + 判定」打包
//! rsw-perms-event/src/listeners.rs  230 行   批量注册 + 生命周期
//! rsw-perms-event/src/dispatch.rs   148 行   派发与去重
//! ```
//!
//! 695 行，干的全是「从事件里取一个字段、按它做判定」。它不该是业务代码。
//! 更糟的是那里面满是 `unwrap_or(0)`：**「载荷里没有 dim」和「dim 是 0（主世界）」
//! 被压成了同一个答案**，而这正是契约 §5.1 记载的那次土地保护绕过 ——
//! 自定义维度的事件读不到 `dim`，全被当成主世界放行，零日志。
//!
//! 所以这里做三件上一代没做的事：
//!
//! 1. [`Event`] 直接给**类型化取值**，并且区分「没有」和「是 0」；
//! 2. **`_unresolved` 感知**：宿主在解不出事件来源时会注入 `_unresolved:[...]`
//!    标记（这是宿主侧修 V-04 时加的）。[`Event::dim`] 一类方法遇到它返回
//!    `Err`，安全判定据此 fail-closed，而不是拿一个编出来的 0 继续走；
//! 3. [`Wiring`] —— 链式批量订阅，句柄由它统一持有。业务侧本来就在自己写。
...
impl<'a> Event<'a> {
    ...
    /// # 为什么返回 `Result`
    ///
    /// 以前它返回 `()`：在**只观察**的事件上调用是「无害的 no-op」。无害是指
    /// 不会崩 —— 但保护类模组会因此以为自己拦住了。「以为拦住了」在这个领域
    /// 比崩溃危险得多：崩溃至少看得见。
    ///
    /// 现在不可取消的事件会直接返回 `Err`，并且**告诉你该去拦哪个**，例如：
    ///
    /// ```text
    /// 事件 `PlayerStartDestroyBlockEvent` 不可取消：它在 origin 之前发、
    /// 只为记录「谁开始挖哪一格」；要拦请用 PlayerDestroyBlockEvent 或
    /// BlockDestroyEvent
    /// ```
    ///
    ...
    /// `Ok`。SDK 不知道的事，不假装知道。
    ///
    /// 返回 `Ok` 只代表「取消位已经写回给宿主」，不代表引擎那边一定停下了：
    /// 有些钩点在半更新的位置，宿主侧对这类点本来就不接受取消。这条边界不是
    /// SDK 能替你验证的，只能靠事件文档。
