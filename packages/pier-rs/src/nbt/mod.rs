...
//! # 为什么这个模块是地基
//!
//! Pier 的 ABI 上没有结构体传参 —— 事件载荷、表单定义与回填、物品、方块状态、
//! 实体快照、命令参数、服务请求/应答，全都是 **SNBT 字符串**。所以「怎么从一段
//! SNBT 里取出一个字段」是模组代码里出现频率最高的一件事，它的手感直接决定
//! 整个 SDK 好不好用。
//!
//! # 与上一代的差别（这不是重写，是把踩过的坑填上）
//!
//! 上一代只给到 `Option<&NbtValue>` 就收工，于是每个消费方都要自己写一层
//! 「取键 → 判类型 → 给默认值」。真实后果可以在业务仓库里看到：一个权限模组
//! 为了读事件载荷手写了 695 行胶水（payload/hit/wiring/dispatch），而且满屏
//! `unwrap_or(0)` —— **「这个键没有」和「这个键的值是 0」被压成了同一个答案**。
//! 土地保护那次绕过事故（事件里读不到 `dim`，消费方 `unwrap_or(0)`，自定义
//! 维度全被当成主世界）就是这么来的。
//!
//! 所以这里同时提供两套取值：
//!
//! * `opt_*` 系列 —— 返回 `Option`，语义是「有就给我，没有我自己兜底」。
//! * `get_*` 系列 —— 返回 `Result`，**缺键和类型不符是两种不同的错误**，且错误
//!   信息里带着键名和实际类型。安全判定（权限、地皮、经济）应当用这一套，
//!   拿到 `Err` 就 fail-closed。
//!
//! 另外补齐了上一代没有的东西：数组下标路径（`a.b[2].c`）、几何类型直读
//! （`as_vec3` / `as_block_pos`）、构造侧的 `From` 与 `obj!` 风格助手、
//! 以及和 `serde_json` 的双向桥（业务侧存档大多是 JSON，来回手抄很费事）。
