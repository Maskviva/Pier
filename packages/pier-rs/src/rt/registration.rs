...
//! # 握手：三道检查，顺序有讲究
//!
//! 和宿主侧（`pier-host/src/ModHost.cpp`）是同一套顺序，两边必须一致：
//!
//! 1. **长度** —— 表短到连核心槽都不覆盖，后面两项都无从谈起（读版本号
//!    本身也是读那块内存）。
//! 2. **版本区间** —— `MIN_SUPPORTED <= 我的版本 <= 宿主版本`。兼容是区间
//!    不是相等（契约 §2.2）。
//! 3. **目标标志** —— `host_flags` 与 `mod_flags` 的 bit 0 必须相等。
//!
//! v0 的做法是把目标标记塞进版本号最高位（`PIER_ABI_TARGET_MASK`），
//! 于是「版本」和「目标」挤在一个字段里，任何一个判断都要先解包。更糟的是
//! 它护不住**没有重编**的模组：那一位在编译期就定死了。现在两者是独立字段，
//! 不匹配时宿主明确拒绝并说明原因，而不是装上去之后在第一个只有一侧才有的
//! 槽上崩掉。
//!
//! # 三个生命周期回调都在 `catch_unwind` 里
//!
//! panic 穿过 `extern "C"` 边界是未定义行为。这里把它拦成一个 `false` 加
//! 一条日志 —— 宿主收到 false 会走正常的装载失败/回滚路径，而不是整个
//! 进程无诊断 abort。
