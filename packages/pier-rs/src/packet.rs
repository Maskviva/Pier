...
//! # 这一层做什么、不做什么
//!
//! 做：把 ABI 的 `packet_hook_*` 包成安全的闭包注册，管住闭包的所有权和
//! panic，把 `{ptr,len}` 收成 `&[u8]`。
//!
//! **不做：解释包体的任何一个字节。** 包 id 是 `i32`，包体是裸字节 ——
//! 版本差异、字段布局、编解码全在调用方那一侧。这不是偷懒：宿主的 ABI
//! 就是这么设计的（见 `abi.h` 的数据包一节），因为一个能跨版本用的 loader
//! 不可能同时懂每个版本的线格式。
//!
//! # 线程（写状态之前先读这一段）
//!
//! ABI 明写着：入站回调跑在**连接被抽水的那个线程**上，出站跑在**发起
//! 发送的那个线程**上。实践中通常是服务器线程，但异步 flush 意味着
//! **不保证**。
//!
//! 所以闭包的约束是 `Send + Sync`，不是 `Send` —— 同一个闭包可能被多个线程
//! 同时进入。要碰世界状态就用 `Host::schedule` 丢回服务器线程；
//! 要维护自己的表就自己上锁。
//!
//! # 派发链
//!
//! 多个订阅者时，每一个看到的是上一个的输出，按注册顺序。**第一个 Drop
//! 胜出**，后面的整个跳过。订阅表在派发前快照，所以回调里注册/注销
//! （包括注销自己）是安全的。
...
impl Packets {
        ...
        F: Fn(&mut Packet<'_>) -> Verdict + Send + Sync + 'static,
    {
        ...
        let unreg = rt().api.packet_hook_unregister;
        ...
        F: Fn(u64, &str, ConnectionState) + Send + Sync + 'static,
    {
        ...
        let unreg = rt().api.packet_conn_hook_unregister;
