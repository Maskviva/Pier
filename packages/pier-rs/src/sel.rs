...
impl PlayerSel {
    ...
    /// 传进 FFI」的形状，不会跨调用留存。
    ///
    /// 现在没有调用点：`PierApi` 里二十多个吃 `PierPlayerSel` 的槽
    /// （`player_send_message` / `player_teleport` / `form_send` …）一个都还没
    /// 包装。用 `expect` 而不是 `allow`：第一个玩家 API 落地、真的调到它的
    /// 那一刻，编译器会报「unfulfilled lint expectation」提醒你把这行删掉。
    #[expect(dead_code)]
