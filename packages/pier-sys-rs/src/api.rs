...
//! # 这份文件是手写的，不是生成的
//!
//! 理由不是洁癖：`abi.h` 的价值有一半在逐槽的注释里（「这个槽为什么长这样」
//! 「上一版怎么错的」），而生成器只搬得动签名、搬不动理由。所以镜像手写，
//! 顺序由 `sys-mirrors-abi` 机检守着 —— 它逐格比对字段名，第一处对不上就
//! 报出「从这一槽起后面全部打到错误的函数指针上」。
//!
//! 那条检查不是可选的。手写镜像的失效方式只有一种，但它是最坏的那一种：
//! `abi.h` 在表尾追加了一个槽而镜像忘了跟，之后每一槽都错位一格，调
//! `bus_publish` 实际打到别的函数指针上，**没有任何诊断**。两侧各自都编得
//! 过，编译器和 clippy 都看不见。
//!
//! # 表里没有任何条件编译
//!
//! 契约 §2.1：布局在所有构建目标下相同，能力缺席 = 槽位 NULL。客户端组
//! （`client_*`）和维度组（`md_*`）永远占位，对应能力包没编进宿主时为
//! `None`。所以这份镜像不需要、也**不许**出现 `#[cfg]`（`sys-mirrors-abi`
//! 会拒绝）—— v0 那次「两侧错位 7 槽」的根因就是镜像和宿主各自分岔。
//!
//! # 每个槽都是 `Option<fn>`
//!
//! 空槽是「这个能力没编进宿主」的**正式表示**，不是异常。Rust 这边用
//! `Option<unsafe extern "C" fn(..)>`，它和裸函数指针布局相同（空指针优化），
//! 所以既拿到了非空检查，又没有多一个字节。
//!
//! 调用之前还要查 `struct_size`：宿主可能比模组老，表短到够不着这个槽 ——
//! 那时候读它是**越界读**，槽位是不是 NULL 都无从谈起。两道闸缺一不可，
//! `pier-rs` 的 `require_slot!` 把它们包在一起。
