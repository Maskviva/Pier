/** world/LiquidFlowEvent.cpp —— 水/岩浆往某一格流。
 *
 * 地皮场景里这是「邻居把水放到我地里」的唯一拦截点：放水本身发生在他自己
 * 的地皮上（PlayerPlaceBlock 拦不住），流过来的那一步才越界。
 *
 * 事件带的是**目标格**（水要流进去的那一格）和来源格，消费方按目标格判权限。
 *
 * 热路径警告：液体每 tick 都在流。判据应当尽量便宜，必要时自己做节流 ——
 * 这个钩子不像压力板那样内置节流缓存，因为「哪一格」比「哪个玩家」变化快得多，
 * 缓存命中率低到不值得。
 */
