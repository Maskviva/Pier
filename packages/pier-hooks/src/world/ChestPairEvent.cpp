 ...
 *
 * 越界配对是一个很少被想到、但真实存在的绕过：把箱子贴着地皮边界放下，让它
 * 和**邻居地里的箱子**配成大箱子，于是打开自己这半边就能看见对面的全部物品
 * ——容器保护判的是「你点的那一格」，而那一格确实是你自己的。
 *
 * 事件带两个箱子的坐标，取消 = 不配对（两个箱子各自独立，原版行为）。
 */
