...
| `dimensions` | 注册自定义维度、格子网格、逐维度规则。可选包。 |
...
`Host` 报三个版本号，它们回答的是不同的问题。把它们搞混这件事已经出过一次事故了。

| | |
|---|---|
| `protocol_version()` | **正在跑的服务端**说的协议。`Err` 的意思是问不出来，见下。 |
| `level_protocol_version()` | **存档**上次被哪个协议写入。它说的是这个世界多老。 |
| `game_sem_version()` | 正在跑的这个构建的 `"major.minor.patch"`。 |

`protocol_version()` 在 Pier 表里没有的 BDS 版本上、以及任何预发布构建上会失败。
那是诚实的答案而不是故障：协议常量在专用服务器上读不到，这个数是从游戏版本推出来的，
而预发布和零售共用同一个三元组却说不同的协议。

它失败而你仍然需要这个数时，读 `game_sem_version()` 自己映射。
**不要退回 `level_protocol_version()`** —— 那是存档的标签，任何比服务端旧的世界上
它都是另一个数，而且是错的。26.40.1 之前 `protocol_version()` 返回的正是那个值，
这也是现在把两者分开的原因。
