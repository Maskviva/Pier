...
class VolumeBuilder:
    ...
    def build(self) -> bytes:
        ...
        # Refused, not recorded. The format has a bit for each and nothing generates
        # either, so a pack carrying one would produce terrain its author did not
        # describe and no line would say so. Writing the bit and hoping is the failure
        # this refusal exists to prevent.
        for key in ("aquifers_enabled", "ore_veins_enabled"):
            if src.get(key, False):
                raise SourceError(
                    f"{key} is not generated by this host, so a pack cannot ask for it. "
                    f"Remove the key, or set it to false, and the terrain will be what the "
                    f"density functions describe."
                )
...
def build_file(source_path: str, out_path: str) -> bytes:
    ...
    _reject_unknown_keys(src, VOLUME_KEYS, "体积包")
